...
export default {
    ...
    execute: async (client, int, embed, emojis) => {
        ...
        const userInfoMenü = new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder()
                .setCustomId(kullanici.id)
                .addOptions([
                    new StringSelectMenuOptionBuilder().setEmoji("🧸").setValue("userInfo-avatar").setLabel("Avatar").setDefault(),
                    new StringSelectMenuOptionBuilder().setEmoji("🌐").setValue("userInfo-profil").setLabel("Profil")
                ])
        )
        ...
        await int.followUp({ embeds, components: [userInfoMenü], });
